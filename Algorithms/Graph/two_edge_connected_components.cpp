...
// Divide all 2ecc into their own component with unique
// id by ignoring bridge edges.
void bridge_tree_dfs(int u, int id) {
    V[u].visited = true;
    V[u].comp_id = id;
    for (auto v : graph[u]) {
...
// Build bridge tree by looping over all edges. If edge is not a bridge
// then both endpoints of that edge will be in the same components so we
// dont have to do anything. Otherwise if its a bridge then get the 
// component id and join those two componenet.
int build_bridge_tree() {
