...
int main() {
    ...
    while (q--) {
        ...
        l = lca(V[u].comp_id, V[v].comp_id);
        // Block cut tree vertices always alternate between articulation point vertex and biconnected
        // component vertex. So if u, v are articulation point then dist[T] = 2 * dist[G] (T = Block
        // cut tree & G = Original graph). But if any one or two of them is/are not an articulation
        // point/s then we need to offset by 1 or 2.
        res = V[V[u].comp_id].depth + V[V[v].comp_id].depth - 2 * V[l].depth + !is_articulation_point[u] +
